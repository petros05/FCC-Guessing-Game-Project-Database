...
DISPALY() {
  ...
  if [[ $USER_ID ]]; then
    #get games played
    GAMES_PLAYED=$($PSQL "SELECT COUNT(user_id) FROM number_guess where user_id = '$USER_ID'")

    #get best game (guess)
    BEST_GUESS=$($PSQL "SELECT MIN(best_game) FROM number_guess WHERE user_id = '$USER_ID' ")
  ...
  echo -e "\nGuess the secret number between 1 and 1000:"
   ...
   while [[ $GUESSED = 0 ]]; do
    ...
    if [[ ! $GUESS =~ ^[0-9]+$ ]]; then
      ...
      #insert into db
      INSERTED_TO_GAMES=$($PSQL "INSERT INTO number_guess(user_id, best_game) VALUES($USER_ID, $TRIES)")
      GUESSED=1
    #if greater
